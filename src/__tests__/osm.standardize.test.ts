...
test('Standardize geojson', () => {
  ...
  test('Get correct file name from OSM geojson', () => {
    expect(standardizeOsm(osmGeoJsonNonWikiImage).features[0].properties!.image).toBeNull();
    ...
    expect(standardizeOsm(osmGeoJsonWikiCategory).features[0].properties!.image).toBeNull();
  });