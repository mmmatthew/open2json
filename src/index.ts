...
export class Provider {
  ...
  public query(sources?: string[], bbox?: BoundingBox, options?:any): Promise<FeatureCollection> {
    ...
    if(sources.length === 0){throw new Error("no sources defined")}
    if(sources.length > 2){throw new Error("Too many sources defined. Maximum two allowed")}
    if(!sources.every(s=>['osm','wikidata'].indexOf(s)>=0)){throw new Error(`unknown source: ${sources}`)}
    ...
    if(geoJsonArray.length === 0){
       throw new Error(
      `Array of GeoJsons returned by promises is empty.`,
    );
